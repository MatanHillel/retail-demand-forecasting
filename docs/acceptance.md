...
| 22 | Presentation and demo video | `docs/presentation.pptx` slide count via python-pptx and `docs/demo.mp4` duration via `ffprobe`. The deck landed with US-38 (12 slides, generated by `scripts/build_presentation.py`); the clause stays PENDING until US-39 delivers the video, because one verdict covers both files. |
